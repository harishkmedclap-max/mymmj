...
export default function Steps() {
  return (
    <section
      id="process"
      className="flex flex-col items-center justify-center px-4 py-16 sm:py-20 bg-[#0b2620]"
    >
      {/* Heading */}
      ...
      <div className="text-center max-w-4xl mx-auto mb-12">
        ...
        <p className="text-muted-inv">
          Get your medical marijuana evaluation with our licensed physicians
          before proceeding with state registration through the Pennsylvania
          Department of Health, which administers the Pennsylvania medical
          marijuana program.
        </p>
      </div>
      {/* Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-6xl mb-10">
        {steps.map(({ id, icon: Icon, title, description }) => (
          ...
          >
            ...
            <h3
              className="heading-quaternary text-white mb-2 
                          transition-colors duration-300 
                          group-hover:text-[#2db882]"
            >
              {title}
            </h3>
